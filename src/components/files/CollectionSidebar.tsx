    ...
    <div className={`fixed left-0 top-16 h-[calc(100vh-4rem)] bg-white border-r-2 border-black z-30 transition-all duration-200 flex flex-col ${open ? 'w-56' : 'w-0 overflow-hidden'}`}>
          ...
          {creating ? (
            <div className="flex gap-1">
              <input
                ...
                className="flex-1 text-xs font-mono px-2 py-1 bg-white border-2 border-black text-black placeholder-neutral-500 font-bold"
                autoFocus
              />
              <button onClick={handleCreate} className="px-2 py-1 bg-black text-white text-xs font-mono font-bold border-2 border-black hover:bg-blue-600 hover:border-blue-600 transition-colors duration-150 cursor-pointer">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        <button
          ...
          className={`w-full text-left px-3 py-2 text-xs font-mono border-2 transition-colors duration-150 cursor-pointer font-bold ${
            !activeCollectionId
